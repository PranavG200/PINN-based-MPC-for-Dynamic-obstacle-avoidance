...
class ManipulatorInformedNN(PINN):
    ...
    def f_model(self, X_f=None):
        ...
        delta = 1/10
        L =1
        i_PR90 = tf.ones(len(t), dtype=tf.float64)
        B_tf = B_tensor(i_PR90)
        # print("*****************************************************************")
        # tf.print(q3[500][0])
        # tf.print(tf.shape(delta*q4*tf.math.sin(q3)))
        # tf.print(tf.shape(delta*q4*tf.math.tan(u)/L))
        # tf.print(tf.shape(delta*u[0]))
        f_pred = tf.concat([delta*q4*tf.math.cos(q3), delta*q4*tf.math.sin(q3), delta*q4*tf.math.tan(u[1])/L, delta*tf.linalg.matvec(B_tf, u[0])], axis =1)
...
if __name__ == "__main__":
    ...
    # Paths
    data_path = os.path.join('/home/vaidehi/Downloads/github/PINNs-based-MPC/resources/data.npz') # ../resources/data.npz')
    weights_path = os.path.join('/home/vaidehi/Downloads/github/PINNs-based-MPC/resources/weights') # '../resources/weights')
