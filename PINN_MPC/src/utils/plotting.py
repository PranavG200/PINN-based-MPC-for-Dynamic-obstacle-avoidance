...
FIGURES_PATH = os.path.join('/home/vaidehi/Downloads/github/PINNs-based-MPC/figures')
FIGURE_WIDTH_PT = 410
...
def plot_states(T, Z_ref, Z_ref2=None, Z_pred=None, Z_mpc=None, filename=None):
    linewidth = 2
    fig = new_fig()
    ax = fig.add_subplot(1, 1, 1)
    ax.set(xlabel='q1', ylabel='q2')
    # ax.set(xlim=[np.min(T), np.max(T)])
    ...
    # ax.plot(Z_ref[:, 0], Z_ref[:, 1], linewidth=linewidth, label='ref') #, c=colors[i * 2])
    # if Z_pred is not None:
    #         ax.plot(Z_pred[:, 0], Z_pred[:, 1], linestyle='--', linewidth=linewidth, label='pred')
    #ax.plot(Z_ref[:, 0], Z_ref[:, 1], linewidth=linewidth, label='q2_ref') #, c=colors[i * 2])
    if Z_pred is not None:
            ax.plot(Z_pred[:, 0], Z_pred[:, 1], linestyle='--', linewidth=linewidth, label='q2_pred')

    ax.plot(Z_ref2[:,0], Z_ref2[:, 1], linewidth=linewidth, label='q2_ref2') #, c=colors[i * 2])
    # if Z_pred is not None:
    #         ax.plot(Z_pred[:, 0], Z_pred[:, 1], linestyle='--', linewidth=linewidth, label='q2_pred')
...
def plot_absolute_error(T, Z_ref, Z_pred=None, Z_mpc=None, filename=None):
    ...
    fig = new_fig()
    ax = fig.add_subplot(1, 1, 1)
    ax.set(xlabel='x', ylabel=r'Absolute Error (rad)')
    ax.set_yscale('log')
    ...
    abs_errors_list = []
    if Z_pred is not None:
        abs_errors = np.abs(Z_ref[:, 1] - Z_pred[:, 1])
        label = 'q1'
    # else:
    #     abs_errors = np.abs(Z_ref[:, i] - Z_mpc[:, i])
    #     label = r'$|' + state + r'^{\mathrm{ref}} - ' + state + r'^{\mathrm{MPC}}|$'
    ...
    mae = abs_errors.mean()
    maxe = abs_errors.max()
    ax.plot(T, abs_errors, linewidth=2, label=label)
    logging.info(label + f', MAE: {mae:.2e}, MaxE: {maxe:.2e}')
